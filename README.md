...
- REPL: `Racket: Open REPL`, `Racket: Run File in REPL`,
  `Racket: Send Selection to REPL`, `Racket: Send Definition to REPL` and
  `Racket: Send S-Expression to REPL` from command palette; send selection sends
  the current line when nothing is selected, send definition sends the top-level
  form at the cursor, where forms inside `module`, `module*` and `module+` count
  as top-level, send s-expression sends the innermost form at the cursor; all
  start a REPL if none is running (needs the [Terminus](https://packagecontrol.io/packages/Terminus) package;
